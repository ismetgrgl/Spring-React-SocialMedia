...
export interface IFetchEditUserPayload{
    token: string,
    name: string,
    about: string,
    phone: string,
...
export const fetchEditUser = createAsyncThunk(
    'user/fetchEditUser', 
   async (payload:IFetchEditUserPayload)=>{
        const res = fetch('http://localhost:9090/user/update',{
            method: 'POST',
            headers:{
                'Content-Type': 'application/json'
            },
...
const userSlice = createSlice({
    ...
    extraReducers: (build)=>{
        ...
        build.addCase(fetchEditUser.fulfilled,(state,action:PayloadAction<IResponse>)=>{
            if(action.payload.code===200){
                state.userProfile = action.payload.data;
            }
        });
